...
function printStochasticStatistics(results,protocols,swarmSizes)
    ...
    fprintf('\n--- EXOGENOUS STOCHASTIC WORKLOAD ---\n');
    fprintf('%-8s %-6s %-22s %-22s %-22s\n', ...
        'Protocol','N','Generated joins','Generated leaves','Generated failures');
    ...
    for p = 1:numel(protocols)
        for n = 1:numel(swarmSizes)
            ...
            fprintf('%-8s %-6d %-22s %-22s %-22s\n', ...
                protocols(p),swarmSizes(n), ...
                meanStdString([x.generatedJoinEvents]), ...
                meanStdString([x.generatedLeaveEvents]), ...
                meanStdString([x.generatedFailureEvents]));
        end
    end

    fprintf('\n--- EXECUTED MEMBERSHIP EVENTS ---\n');
    fprintf('%-8s %-6s %-22s %-22s %-22s\n', ...
        'Protocol','N','Joins','Total leaves','Failures');
    ...
    fprintf('\n--- DT-HSBP ACCOUNTING ---\n');
    fprintf('%-6s %-20s %-20s %-20s %-20s %-20s %-20s\n', ...
        'N','DT disturbances','Predictions','Realized','Unrealized', ...
        'DT-driven leaves','Collateral leaves');

    for n = 1:numel(swarmSizes)
        x = selectGroup(results,"DTHSBP",swarmSizes(n));
        fprintf('%-6d %-20s %-20s %-20s %-20s %-20s %-20s\n', ...
            swarmSizes(n), ...
            meanStdString([x.dtDisturbanceCount]), ...
            meanStdString([x.dtPredictionsCreated]), ...
            meanStdString([x.dtPredictionsRealized]), ...
            meanStdString([x.dtPredictionsUnrealized]), ...
            meanStdString([x.dtDrivenLeaveEvents]), ...
            meanStdString([x.dtCollateralDepartureEvents]));
    end

    fprintf('\n--- DT-HSBP ---\n');
    fprintf('%-6s %-22s %-22s %-22s %-22s\n', ...
        'N','Predicted leaves','Batch rekeys','DT realization ratio','Final active UAVs');

    for n = 1:numel(swarmSizes)
        x = selectGroup(results,"DTHSBP",swarmSizes(n));
        fprintf('%-6d %-22s %-22s %-22s %-22s\n', ...
            swarmSizes(n), ...
            meanStdString([x.predictedLeaves]), ...
            meanStdString([x.batchRekeys]), ...
            meanStdString([x.dtRealizationRatio]), ...
